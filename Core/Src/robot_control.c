...
void setLocalTargetSpeed(RobotCommandV2 * ai_cmd, target_t * target, imu_t * imu)
{
  if (ai_cmd->control_mode == SIMPLE_VELOCITY_TARGET_MODE) {
    ...
    temp_gloval_vel[0] = ai_cmd->mode_args.velocity.target_global_velocity_r * cos(ai_cmd->mode_args.velocity.target_global_velocity_theta);
    temp_gloval_vel[1] = ai_cmd->mode_args.velocity.target_global_velocity_r * sin(ai_cmd->mode_args.velocity.target_global_velocity_theta);

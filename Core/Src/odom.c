...
{
  // motor->enc_angle_rad,/imu->yaw_rad

  for (int i = 0; i < 4; i++) {
    if (isnan(motor->enc_angle_rad[i])) {
      motor->enc_angle_rad[i] = 0;
    }
    motor->angle_diff[i] = getAngleDiff(motor->pre_enc_angle_rad[i], motor->enc_angle_rad[i]);
...
void inntegOdomUpdate(RobotCommandV2 * ai_cmd, omni_t * omni, integ_control_t * integ, connection_t * connection, imu_t * imu, system_t * sys)
{
  ...
  for (int i = 0; i < 2; i++) {
    ...
    integ->global_odom_vision_diff[i] = sumNewestN(integ->odom_log[i], integ->latency_cycle) / MAIN_LOOP_CYCLE;
